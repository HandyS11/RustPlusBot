...
public sealed class TeamStateTrackerTests
{
    ...
    public void Null_snapshot_emits_nothing_and_keeps_baseline()
    {
        ...
        var result = tracker.Diff(Team(1, Member(1, online: false)), DateTimeOffset.UnixEpoch, TimeSpan.FromMinutes(5),
            1f);
    ...
    public void Brand_new_member_is_primed_silently()
    {
        ...
        var result = tracker.Diff(Team(1, Member(1), Member(2, online: true)), DateTimeOffset.UnixEpoch,
            TimeSpan.FromMinutes(5), 1f);
    ...
    public void Connect_detected_on_offline_to_online()
    {
        ...
        var result = tracker.Diff(Team(1, Member(1, online: true)), DateTimeOffset.UnixEpoch, TimeSpan.FromMinutes(5),
            1f);
    ...
    public void Death_detected_on_deathtime_advance_even_if_alive_again()
    {
        ...
        var result = tracker.Diff(Team(1, Member(1, alive: true, death: t0.AddMinutes(1))), DateTimeOffset.UnixEpoch,
            TimeSpan.FromMinutes(5), 1f);
    ...
    public void Respawn_detected_on_spawntime_advance_with_current_location()
    {
        ...
        var result = tracker.Diff(Team(1, Member(1, x: 50, y: 60, spawn: t0.AddMinutes(1))), DateTimeOffset.UnixEpoch,
            TimeSpan.FromMinutes(5), 1f);
    ...
    public void Connect_and_disconnect_have_no_location()
    {
        var tracker = new TeamStateTracker();
        tracker.Diff(Team(1, Member(1, online: false)), DateTimeOffset.UnixEpoch, TimeSpan.FromMinutes(5), 1f);
        var result = tracker.Diff(Team(1, Member(1, online: true, x: 9, y: 9)), DateTimeOffset.UnixEpoch,
            TimeSpan.FromMinutes(5), 1f);
    ...
    public void Leader_death_uses_deathnote_location()
    {
        var t0 = DateTimeOffset.UnixEpoch;
        var tracker = new TeamStateTracker();
        tracker.Diff(new TeamInfoSnapshot(1, [Member(1, death: t0)]), DateTimeOffset.UnixEpoch, TimeSpan.FromMinutes(5),
            1f);
        var snap = new TeamInfoSnapshot(1, [Member(1, x: 1, y: 1, death: t0.AddMinutes(1))], (700f, 800f));
        var death = Assert.Single(tracker.Diff(snap, DateTimeOffset.UnixEpoch, TimeSpan.FromMinutes(5), 1f),
            t => t.Kind == PlayerTransitionKind.Death);
    ...
    public void Nonleader_death_uses_previous_poll_position_not_current()
    {
        ...
        tracker.Diff(new TeamInfoSnapshot(1, [Member(1), Member(2, x: 10, y: 10, death: t0)]), DateTimeOffset.UnixEpoch,
            TimeSpan.FromMinutes(5), 1f);
    ...
    public void Death_with_no_prior_position_and_no_deathnote_has_null_location()
    {
        ...
        var first = tracker.Diff(new TeamInfoSnapshot(1, [Member(1), Member(2, death: t0)]), DateTimeOffset.UnixEpoch,
            TimeSpan.FromMinutes(5), 1f);
