...
public sealed class AlarmChannelLocatorTests
{
    private static (AlarmChannelLocator Locator, ServiceProvider Provider, string ConnectionString, IClock Clock)
        CreateLocator()
    {
        var clock = Substitute.For<IClock>();
        clock.UtcNow.Returns(DateTimeOffset.UnixEpoch);

        var cs = $"DataSource=alarm-locator-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
        ...
        var locator = new AlarmChannelLocator(provider.GetRequiredService<IServiceScopeFactory>(), clock);
    ...
    private static async Task<Guid> SeedAsync(string connectionString)
    {
        ...
        context.ProvisionedChannels.Add(new ProvisionedChannel
        {
            ...
            ChannelKey = WorkspaceChannelKeys.ServerAlarms,
    ...
    [Fact]
    public async Task GetChannelIdAsync_returns_provisioned_alarms_channel()
    {
