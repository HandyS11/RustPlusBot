...
public sealed class StorageMonitorChannelLocatorTests
{
    private static (StorageMonitorChannelLocator Locator, ServiceProvider Provider, string ConnectionString, IClock
        Clock)
        CreateLocator()
    {
        ...
        var cs = $"DataSource=storagemonitor-locator-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
        ...
        var locator = new StorageMonitorChannelLocator(provider.GetRequiredService<IServiceScopeFactory>(), clock);
    ...
    private static async Task<Guid> SeedAsync(string connectionString)
    {
        ...
        context.ProvisionedChannels.Add(new ProvisionedChannel
        {
            ...
            ChannelKey = WorkspaceChannelKeys.ServerStorageMonitors,
            DiscordChannelId = 888UL,
            CreatedAt = DateTimeOffset.UnixEpoch,
        });

        // A row with a null RustServerId (global scope) must be skipped by the locator.
        context.ProvisionedChannels.Add(new ProvisionedChannel
        {
            GuildId = 10UL,
            RustServerId = null,
    ...
    [Fact]
    public async Task GetChannelIdAsync_returns_provisioned_storagemonitors_channel()
    {
