...
public sealed class EventChannelLocatorTests
{
    private static (EventChannelLocator Locator, ServiceProvider Provider, string ConnectionString, IClock Clock)
        CreateLocator()
    {
        var clock = Substitute.For<IClock>();
        clock.UtcNow.Returns(DateTimeOffset.UnixEpoch);

        var cs = $"DataSource=event-locator-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
        ...
        var locator = new EventChannelLocator(provider.GetRequiredService<IServiceScopeFactory>(), clock);
    ...
    [Fact]
    public async Task GetChannelIdAsync_returns_provisioned_events_channel()
    {
