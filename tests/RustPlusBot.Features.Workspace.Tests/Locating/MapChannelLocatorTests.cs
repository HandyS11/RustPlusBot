...
public sealed class MapChannelLocatorTests
{
    private static (MapChannelLocator Locator, ServiceProvider Provider, string ConnectionString, IClock Clock)
        CreateLocator()
    {
        var clock = Substitute.For<IClock>();
        clock.UtcNow.Returns(DateTimeOffset.UnixEpoch);

        var cs = $"DataSource=map-locator-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
        ...
        var locator = new MapChannelLocator(provider.GetRequiredService<IServiceScopeFactory>(), clock);
    ...
    private static async Task<Guid> SeedAsync(string connectionString)
    {
        ...
        context.ProvisionedChannels.Add(new ProvisionedChannel
        {
            ...
            ChannelKey = WorkspaceChannelKeys.ServerMap,
    ...
    [Fact]
    public async Task GetChannelIdAsync_returns_provisioned_map_channel()
    {
